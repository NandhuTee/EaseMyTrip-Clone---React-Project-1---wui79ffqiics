...
import FlightIcon from "@mui/icons-material/Flight"; // Import FlightIcon from Material-UI icons
...
const FlightsTo = ({ onclose }) => {
  ...
  const handleLiData = (city, name, iata_code) => {
    setAriportTo([city, name, iata_code]);
    setLiData(true);
    onclose(liData);
  };
          ...
          {airportDetail.map((data, index) => (
            ...
            >
              <div className={Classes.listFlightTo}>
                <FlightIcon className={Classes.flightIcon} /> {/* FlightIcon */}
